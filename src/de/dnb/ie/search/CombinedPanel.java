...
public class CombinedPanel extends GridPanel implements IPanelModel {
	/**
	 * 
	 */
	...
	public CombinedPanel(
		...
		this.textField.addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent arg0) {
				searchModel.setFocusModel(CombinedPanel.this);				
			}
		});
	}
	...
	public String getSearchPhrase() {
		...
		if (fieldText.trim().length() > 0) {
			...
			for (int i = 0; i < searchIndexes.length; i++) {
				String index = searchIndexes[i];
				text += index + " (" + fieldText + ") ";
